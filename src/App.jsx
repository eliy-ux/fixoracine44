...
function Player({ item, onClose }) {
  if (!item) return null;
  return <AnimatePresence><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-black/95 p-3 sm:p-8"><button onClick={onClose} className="absolute right-5 top-5 z-10 rounded-full bg-white/10 p-3 hover:bg-red-500"><X size={20} /></button><div className="grid h-full place-items-center"><div className="w-full max-w-6xl overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl"><div className="flex items-center justify-between border-b border-white/10 px-5 py-4"><span className="font-bold">Now playing <span className="text-white/50">· {item.title}</span></span><span className="text-xs text-white/40">Fixora Cine player</span></div><div className="aspect-video"><iframe title={`Playing ${item.title}`} src={`https://vidsrc.to/embed/${item.type === 'tv' ? 'tv' : 'movie'}/${item.id}`} className="h-full w-full border-0" allowFullScreen /></div></div></div></motion.div></AnimatePresence>;
}
...
export default function App() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#0a0a0f] text-white selection:bg-red-500/40">
    ...
    <main id="top" className="relative z-10">
      ...
      <div id="trending"><Section title={activeGenre === 'All' ? 'Trending now' : `${activeGenre} picks`} icon="✦" items={genreItems.length ? genreItems : filteredTrending} onOpen={setSelected} onPlay={setPlaying} watchlist={watchlist} onToggle={toggleWatchlist} loading={loading} /></div>
      ...
      <Section title="Top rated films" icon="★" items={movies} onOpen={setSelected} onPlay={setPlaying} watchlist={watchlist} onToggle={toggleWatchlist} loading={loading} />
      <Section title="New releases & series" icon="◈" items={shows} onOpen={setSelected} onPlay={setPlaying} watchlist={watchlist} onToggle={toggleWatchlist} loading={loading} />
      {watchlist.length > 0 && <Section title="Your watchlist" icon="♡" items={watchlist} onOpen={setSelected} onPlay={setPlaying} watchlist={watchlist} onToggle={toggleWatchlist} />}
    </main>
    ...
    <Modal item={selected} onClose={() => setSelected(null)} onPlay={item => { setSelected(null); setPlaying(item); }} inWatchlist={selected ? watchlist.some(x => x.id === selected.id) : false} onToggle={toggleWatchlist} />
