...
function Player({ item, onClose, onResume }) {
  ...
  const source = server === 'embedmaster'
    ...
    : server === 'embedsu'
      ...
      : server === '2embed'
        ? item.type === 'movie' ? `https://www.2embed.cc/embed/${item.id}` : `https://www.2embed.cc/embedtv/${item.id}&s=${season}&e=${episode}`
        : item.type === 'movie' ? `https://vidsrc.to/embed/movie/${item.id}` : `https://vidsrc.to/embed/tv/${item.id}/${season}/${episode}`;
  const updateEpisode = (nextSeason, nextEpisode) => { setSeason(nextSeason); setEpisode(nextEpisode); setIsLoading(true); setFailed(false); onResume?.(item, nextSeason, nextEpisode); };
  const changeServer = nextServer => { setServer(nextServer); setIsLoading(true); setFailed(false); };
  return <AnimatePresence><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] overflow-y-auto bg-black/95 p-3 sm:p-8"><button onClick={onClose} className="fixed right-5 top-5 z-10 rounded-full bg-white/10 p-3 hover:bg-red-500"><X size={20} /></button><div className="mx-auto flex min-h-full max-w-6xl items-center"><div className="w-full overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-5 py-4"><span className="font-bold">Now playing <span className="text-white/50">· {item.title}{item.type === 'tv' ? ` (S${season} E${episode})` : ''}</span></span><div className="flex items-center gap-2"><label className="text-xs text-white/45">Server</label><select value={server} onChange={e => changeServer(e.target.value)} className="rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-xs text-white outline-none"><option value="vidsrc">VidSrc</option><option value="embedmaster">EmbedMaster</option><option value="embedsu">EmbedSu</option><option value="2embed">2Embed</option></select>{item.type === 'tv' && <><select value={season} onChange={e => updateEpisode(Number(e.target.value), episode)} className="rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-xs text-white outline-none">{Array.from({ length: 15 }, (_, i) => <option key={i + 1} value={i + 1}>Season {i + 1}</option>)}</select><select value={episode} onChange={e => updateEpisode(season, Number(e.target.value))} className="rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-xs text-white outline-none">{Array.from({ length: 30 }, (_, i) => <option key={i + 1} value={i + 1}>Episode {i + 1}</option>)}</select></>}</div></div><div className="relative aspect-video bg-[#050509]"><iframe key={source} title={`Playing ${item.title}`} src={source} onLoad={() => setIsLoading(false)} onError={() => { setIsLoading(false); setFailed(true); }} className="h-full w-full border-0" allowFullScreen />{isLoading && <div className="absolute inset-0 grid place-items-center bg-[#08080d] text-sm text-white/55"><div className="text-center"><div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-2 border-white/15 border-t-red-400" /><p>Connecting to {server}…</p></div></div>}{failed && <div className="absolute inset-0 grid place-items-center bg-[#08080d] p-6 text-center"><div><p className="font-bold">This player service did not respond.</p><p className="mt-2 text-sm text-white/50">Choose another authorized player service above and try again.</p></div></div>}</div></div></div></motion.div></AnimatePresence>;
}
